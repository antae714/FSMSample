...
FText UFSMTransitionNode::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	...
	if ((PrevState != NULL) && (NextState != NULL))
	{
		...
		Args.Add(TEXT("PrevState"), FText::FromName(PrevState->GetNodeName()));
		Args.Add(TEXT("NextState"), FText::FromName(NextState->GetNodeName()));
...
void UFSMTransitionNode::Compile(FKismetFunctionContext& Context) const
{
	...
	int index = BPClass->Transitions.Emplace(GetNodeName(), NodeGuid);
	FFSMTransitionClass& FSMTransitionClass = BPClass->Transitions[index];
	FSMTransitionClass.NextNodeGUID = GetNextState()->NodeGuid;
}
