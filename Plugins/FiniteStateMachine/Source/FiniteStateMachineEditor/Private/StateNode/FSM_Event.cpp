...
public:
	...
	void SettingState(FKismetFunctionContext& Context, UEdGraphNode* Node)
	{
		...
		if (findfuction)
		{
			...
			if (!state) return;
			if (EventNode->DelegateName == StateFunction::EntryName)
			{
				state->StateEntry = findfuction;
			}
			else if (EventNode->DelegateName == StateFunction::UpdateName)
			{
				state->StateUpdate = findfuction;
			}
			else if (EventNode->DelegateName == StateFunction::ExitName)
			{
				state->StateExit = findfuction;
			}
...
void UFSM_Event::SetDelegateName(FName NewDelegateName)
{
